...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item_name === '' || $item_quantity < 0 || $item_category === '') {
        echo json_encode(['status' => 'error', 'message' => 'Please fill in all fields correctly.']);
        exit;
    }

    $stmt = $conn->prepare("INSERT INTO tbl_inventory (item_name, item_quantity, item_category, item_status, created_at) VALUES (?, ?, ?, 'In Stock', NOW())");
    $stmt->bind_param("sis", $item_name, $item_quantity, $item_category);

    if ($stmt->execute()) {
        $inventory_id = $stmt->insert_id;

        // Prepare plain text details for log
        $detailsText = "Item: $item_name; Quantity: $item_quantity; Category: $item_category;";
